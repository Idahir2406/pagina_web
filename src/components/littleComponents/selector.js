...
export const Selector = ({options,selected,setSelected}) => {
  ...
  return (
    <Listbox value={selected} onChange={setSelected} as="div">
      ...
      <TransitionComponent>
        <Listbox.Options className="absolute mt-2 origin-top-right bg-white divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="px-1 py-1">
            {options.map((option,index) => (
              <Listbox.Option
                className="bg-white text-black hover:bg-violet-200 hover:text-white
