...
export default async function handler(req:Request, res:Response) {
  ...
  if (method === "POST") {
    ...
    const contextMessage = {
      role: "system",
      content: `Eres un asistente virtual o chatbot de una tienda de venta de productos de navidad llamada Christmas Store, y los que están disponibles son: ${productos.slice(0,10).map(producto => `${producto.name} (${producto.description}) - Precio: ${producto.price} - Categoría: ${producto.category} `).join(', ')}. Solo puedes dar respuestas relacionadas con ese tema y con los productos. Si alguien hace una pregunta fuera del tema de la tienda, da un aviso de que no puedes cambiar el tema. No devuelvas links, ni urls, el resto si. No responderás a preguntas fuera de este contexto.
